    ...
    def add(self,widget):
        ...
        if(widget_type==TextArea or isinstance(widget,TextArea)):
            ...
            if(widget.fontsize is not None):
                font1 = wx.Font(widget.fontsize, wx.NORMAL, wx.NORMAL, wx.NORMAL, False)
            ...
            self.button1 = wx.BitmapButton(self.panel, id=-1, bitmap=img,pos=widget.position, size = widget.size)
...
class Dialog(wx.MessageDialog):
    controller = None
    def __init__(self,title,text):pass
       # self.title = title
       # self.text = text
    ...
    def show(self,width,height,title,text):
        self.width = width
        self.height = height
        self.controller = wx.MessageDialog(None, text, title, wx.OK | wx.ICON_INFORMATION) 
        self.controller.ShowModal()
        #self.controller.SetSize(self.width, self.height)
...
class LabelText(wx.StaticText):
    controller = None
    def __init__(self,text,X,Y,width,height,fontsize=None):
        self.text = text
        self.position = (X,Y)
...
if __name__ == '__main__':
    ...
    img = Image('test.png',100,100,100,100)
    canvas.add(img)
