...
''' fltk_Canvas : On it widget can be added '''
	...
	def add(self,widget):
		...
		if(widget_type==TextArea or isinstance(widget,TextArea)):
			...
			widget.controller.value(widget.title)
	                ...
	                widget.controller = Fl_Input(widget.position_X, widget.position_Y, widget.width, widget.height)
			...
			widget.controller.copy(widget.choices)
            		...
            		widget.controller = Fl_Box(widget.position_X, widget.position_Y, widget.width, widget.height, widget.title)
			widget.controller.labelsize(widget.size)

		elif(widget_type==Slider or isinstance(widget,Slider)):
            		widget.controller = Fl_Hor_Value_Slider(widget.position_X, widget.position_Y, widget.width, widget.height, widget.text) 
                        widget.controller.range(widget._from, widget._to)
                        #widget.precision(1)
...
''' WIDGETS: LabelText '''
class LabelText(Fl_Box):
    controller = None
    def __init__(self,title, X,Y,width,height,fontsize):
	self.title = title
        self.position_X = X
	self.position_Y = Y
        self.width = width
...
class Dialog(object):
	...
	self.window = Fl_Window(0,500,width,height,title)
	self.box = Fl_Box(20,40,260,100,text)
        self.window.position((Fl.w() - self.window.w())/2, (Fl.h() - self.window.h())/2);

	#self.box.box(FL_UP_BOX)
	#self.box.labelsize(36)
	#self.box.labelfont(FL_BOLD+FL_ITALIC)
	#self.box.labeltype(FL_SHADOW_LABEL)
	self.window.end()
	self.window.show(sys.argv)
...
''' WIDGETS: Slider '''
class Slider(Fl_Hor_Value_Slider):#Fl_Value_Slider):
...
if __name__ == '__main__':
	...
	#Constructor canvas
	canvas = Canvas(1, 'fltkGUI -v1.0 | Kamaldeep Singh' ,510,300)
