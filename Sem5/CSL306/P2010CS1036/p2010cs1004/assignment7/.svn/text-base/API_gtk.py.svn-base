...
class WindowFrame:
	...
	####
	    ...
	    widget.controller.set_range(widget.start, widget.end)#setting the range of Slider Widget
	    widget.controller.set_size_request(widget.width,widget.height)#setting the size of Slider 
	    self.fixed.put(widget.controller, widget.position_X, widget.position_Y)#setting the position of Slider
	    widget.controller.show()
            
	
	elif(WidgetName==SpinBox):#if widget added is SpinBox
	    widget.controller = gtk.SpinButton()#getting a SpinButton for SpinBox
	    widget.controller.set_range(widget.start,widget.end)#setting the range of SpinBox Widget
	    widget.controller.set_increments(1, 1)#setting the increment for SpinBox
	    widget.controller.set_size_request(widget.width,widget.height)##setting the size of SpinBox
	    self.fixed.put(widget.controller, widget.position_X, widget.position_Y)#setting the size of SpinBox
	    widget.controller.show()
	...
	###
	    ...
	    store = gtk.ListStore(str)#making a store variable to store the values Of SelectList
	    for act in widget.choices:
            	store.append([act]) #storing the values in store variable 
	    widget.controller = gtk.TreeView(store)# making a TreeView for the values in List
	    widget.controller.show()
	    widget.controller.set_size_request(widget.width,widget.height)#setting the size of SelectList
            self.fixed.put(widget.controller,widget.position_X, widget.position_Y)#setting the position of SelectList
	    rendererText = gtk.CellRendererText()
            column = gtk.TreeViewColumn("", rendererText, text=0)#making columns for the values stored in store 
            column.set_sort_column_id(0)    
            widget.controller.append_column(column)#appending the columns to SelectList
...
#SelectList serves as a list containing values and it can trigger a function if we select one of the values            
class SelectList(object): 
	controller = None #controller i.e  the actual widget will be assigned when it is added to the WindowFrame
	
        def __init__(self,choices,X,Y,width,height,value=""):#constructor to store choices in list x-Y positions and width and height
	...
	def clickListener(self,method): # this functions assigns a method to the List which will be triggered when we select one of the options
	
	     if(self.controller == None):
		
	         self.callbackMethod = method #storing the callback method since list is stil not present
		
	     else:
		 def on_activated(widget, row, col):
        		model = widget.get_model()
        		paragraph = model[row][0]
        		method(paragraph)
		 self.controller.connect("row-activated",on_activated) # 
...
# Slider object has a slider between range of values
class Slider(object):
    ...
    def __init__(self,start,end,X,Y,width,height):#setting the style attributes for Slider
    ...
    def getValue(self):# returns the value of Slider
	
	if(self.controller == None):
	    return ''
	else:	
	    return self.controller.get_value()
#SpinBox spins the values between two values and has function to get the value of SpinBox   
class SpinBox(object):
    ...
    def __init__(self,start,end,X,Y,width,height):#storing the style attributes
    ...
    def getValue(self):#returns the value of SpinBox
